...
public enum ProblemStatus implements Status {
  ...
  private static final Map<Integer, ProblemStatus> STATUSES = Arrays.stream(values())
      .collect(toMap(ProblemStatus::value, Function.identity()));

  private final int value;
  private final String reasonPhrase;

  ProblemStatus(int value, String reasonPhrase) {
    this.value = value;
    this.reasonPhrase = reasonPhrase;
  }

  @Nullable
  public static ProblemStatus valueOf(final int code) {
    @Nullable final ProblemStatus status = STATUSES.get(code);
    return status;
  }
